...
export function SearchBar() {
  ...
  const filterPills = [
    { label: '[TSX]', value: '[tsx]', title: 'Filter to React View components' },
    { label: '[Backend]', value: '[backend]', title: 'Filter to Backend/Rust files' },
    { label: '[External]', value: '[external]', title: 'Filter to External dependencies' },
    { label: '[Circular]', value: '[circular]', title: 'Filter to Circular import loops' },
    { label: '[Unused]', value: '[unused]', title: 'Filter to Unused/Dead code files' },
  ];

  const handlePillClick = (val: string) => {
    if (searchQuery.trim().toLowerCase() === val.toLowerCase()) {
      setSearchQuery('');
    } else {
      setSearchQuery(val);
    }
  ...
  return (
    <div className="absolute top-4 right-4 z-20 flex flex-col items-end gap-2 animate-slide-up select-none">
      ...
      <div className="flex items-center gap-1.5 px-1 animate-fade-in">
        {filterPills.map((pill) => {
          const isActive = searchQuery.trim().toLowerCase() === pill.value.toLowerCase();
          return (
            <button
              key={pill.value}
              onClick={() => handlePillClick(pill.value)}
              title={pill.title}
              className={`px-2 py-0.5 rounded border transition-all cursor-pointer font-bold text-[9px] font-mono shadow-sm select-none hover:scale-105 ${
                isActive
                  ? 'bg-blue-500/20 text-blue-400 border-blue-500/40 shadow-blue-500/10'
                  : 'bg-surface/80 hover:bg-surface-raised border-border-subtle text-text-dim hover:text-text-main'
              }`}
            >
              {pill.label}
            </button>
